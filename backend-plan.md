...
- ✅ **User** - користувачі (ПІБ, місто, посада, логін, пароль, статистика, монети "Мрійчики", avatarUrl - фото аватара)
- ✅ **Category** - категорії питань
- ✅ **Question** - питання (текст, категорія, варіанти відповідей, правильна відповідь, пояснення, зображення)
- ⏳ **Question** - додати поля: type (multiple_choice/text_input), mediaType (none/image/video), mediaUrl, correctAnswers (масив правильних відповідей для text_input), knowledgeArticle (посилання на статтю з бази знань для аналізу неправильних відповідей)
- ✅ **DailyTest** - щоденні тести (користувач, дата, рівно 5 питань, статус, дедлайн)
- ⏳ **DailyTest** - додати поля: startTime (час початку тесту), timeLimit (ліміт часу в хвилинах, за замовчуванням 20), remainingTime (залишковий час)
...
- ✅ **KnowledgeBase** - додано поле: positions (масив посада, до яких прив'язана стаття) ✅
...
- ✅ **City** - міста
- ✅ **Position** - посади
- ⏳ **Position** - додати поле: knowledgeArticles (масив статей бази знань, прив'язаних до посади)
...
- ⏳ **FacilityCompetition** - змагання між закладами (facility1, facility2, startDate, endDate, status, winner, facility1CorrectPercentage, facility2CorrectPercentage, facility1EmployeesCount, facility2EmployeesCount)
- ⏳ **SystemSettings** - налаштування системи (knowledgeBaseAccessEnabled - доступ до бази знань включений/виключений, testTimeLimit - ліміт часу на тест в хвилинах, за замовчуванням 20)
...
- ⏳ `POST /api/users/:id/avatar` - завантаження фото аватара користувача (multipart/form-data)
- ⏳ `DELETE /api/users/:id/avatar` - видалення фото аватара користувача
...
- ✅ `POST /api/questions` - створення питання (адмін)
  - ⏳ Підтримка вибору типу питання (multiple_choice/text_input)
  - ⏳ Завантаження фото/відео до питання (поля: mediaType, mediaFile)
  - ⏳ Налаштування правильних відповідей для питань з введенням (масив correctAnswers)
  - ⏳ Прив'язка статті з бази знань до питання (поле knowledgeArticle)
- ✅ `PUT /api/questions/:id` - оновлення питання (адмін)
  - ⏳ Оновлення типу питання, медіа, правильних відповідей, прив'язки до статті
- ✅ `DELETE /api/questions/:id` - видалення питання (адмін)
- ✅ Завантаження зображення через `POST /api/questions` з полем `image`
- ⏳ Завантаження фото/відео через `POST /api/questions` з полем `mediaFile` (multipart/form-data)
...
- ⏳ `POST /api/daily-tests/:id/start` - початок тесту (збереження startTime, встановлення timeLimit 20 хвилин)
- ⏳ `GET /api/daily-tests/:id/remaining-time` - отримання залишкового часу на проходження тесту
- ✅ `POST /api/daily-tests/:id/answer` - відправка відповіді
  - ⏳ Підтримка відповідей для питань з варіантами (answerId)
  - ⏳ Підтримка відповідей для питань з введенням тексту (answerText - перевірка через алгоритм Левенштейна)
- ✅ `GET /api/daily-tests/:id/results` - результати тесту
- ⏳ `GET /api/daily-tests/:id/wrong-answers-analysis` - аналіз неправильних відповідей з посиланнями на статті з бази знань
...
- ✅ `GET /api/knowledge-base` - список статей (з пошуком та пагінацією)
  - ✅ Перевірка доступу до бази знань (якщо доступ закритий - повертати помилку або порожній список) - реалізовано в мобільному додатку
  - ✅ Фільтрація статей по посаді користувача (якщо статті прив'язані до посад) - реалізовано в мобільному додатку
- ✅ `GET /api/knowledge-base/:id` - деталі статті (з автоматичним збільшенням переглядів)
  - ✅ Перевірка доступу до бази знань - реалізовано в мобільному додатку
  - ✅ Перевірка прив'язки статті до посади користувача - реалізовано в мобільному додатку
- ✅ `GET /api/knowledge-base/category/:categoryId` - статті по категорії
- ✅ `POST /api/knowledge-base` - створення статті (адмін)
  - ✅ Прив'язка статті до посад (масив positions) - реалізовано в адмін панелі
- ✅ `PUT /api/knowledge-base/:id` - оновлення статті (адмін)
  - ✅ Оновлення прив'язки до посад - реалізовано в адмін панелі
- ✅ `DELETE /api/knowledge-base/:id` - видалення статті (адмін)
- ✅ `GET /api/knowledge-base/access-status` - перевірка статусу доступу до бази знань - реалізовано в мобільному додатку
...
- ✅ `PUT /api/positions/:id` - оновлення посади (адмін)
  - ⏳ Прив'язка до статей бази знань (масив knowledgeArticles)
...
### Змагання між закладами (Facility Competitions):

- ⏳ `GET /api/facility-competitions` - список змагань (активні, завершені)
- ⏳ `GET /api/facility-competitions/:id` - деталі змагання
- ⏳ `POST /api/facility-competitions` - створення змагання (користувач вибирає заклад-противник)
- ⏳ `GET /api/facility-competitions/:id/progress` - прогрес обох закладів (відсоток правильних відповідей, кількість працівників)
- ⏳ `GET /api/facility-competitions/active` - активні змагання користувача
- ⏳ `GET /api/facility-competitions/history` - історія змагань
- ⏳ `GET /api/facilities` - список закладів для вибору при створенні змагання
- ⏳ `GET /api/facility-competitions/:id/winner` - визначення переможця (заклад з найвищим відсотком правильних відповідей)

### Налаштування системи (System Settings):

- ⏳ `GET /api/settings` - отримання налаштувань системи (доступ до бази знань, ліміт часу на тест)
- ⏳ `PUT /api/settings/knowledge-base-access` - увімкнення/вимкнення доступу до бази знань (адмін)
- ⏳ `PUT /api/settings/test-time-limit` - налаштування ліміту часу на проходження тесту (адмін, за замовчуванням 20 хвилин)
...
- ✅ Автоматична перевірка та нарахування ачівок після завершення тесту
- ⏳ **Ліміт часу 20 хвилин на проходження тесту:**
  - Збереження часу початку тесту (startTime) при виклику POST /api/daily-tests/:id/start
  - Встановлення ліміту часу (timeLimit = 20 хвилин за замовчуванням, налаштовується в SystemSettings)
  - Розрахунок залишкового часу (remainingTime = timeLimit - (поточний час - startTime))
  - Автоматичне завершення тесту при досягненні 0 хвилин
  - Перевірка залишкового часу перед прийняттям кожної відповіді
- ⏳ **Підтримка різних типів питань:**
  - Тип "multiple_choice" - стандартна логіка (1 питання, 4 варіанти відповідей)
  - Тип "text_input" - питання з полем для введення відповіді
  - Перевірка відповіді для text_input через алгоритм Левенштейна (співставлення з масивом correctAnswers)
  - Збереження правильної відповіді (для multiple_choice - answerId, для text_input - правильна відповідь з correctAnswers)
- ⏳ **Підтримка фото/відео в питаннях:**
  - Завантаження фото/відео файлів при створенні питання
  - Збереження URL медіа файлу (mediaUrl) та типу (mediaType: none/image/video)
  - Відображення медіа контенту в мобільному додатку
- ⏳ **Аналіз неправильних відповідей:**
  - Після завершення тесту генерувати список неправильних відповідей
  - Для кожного неправильного питання, якщо є прив'язка до статті (knowledgeArticle), повертати посилання на статтю
  - Endpoint GET /api/daily-tests/:id/wrong-answers-analysis повертає масив неправильних питань з посиланнями на статті
...
- ⏳ Завантаження відео файлів до питань (валідація типів: mp4, mov, avi)
- ⏳ Завантаження фото аватара користувача (валідація типів: jpg, jpeg, png, webp)
- ⏳ Обрізка та оптимізація аватара перед збереженням

## 1.7.1. Логіка змагань між закладами ⏳

- ⏳ **Створення змагання:**
  - Користувач вибирає заклад-противник зі списку доступних закладів
  - Створюється FacilityCompetition з facility1 (заклад користувача), facility2 (вибраний заклад)
  - Встановлюються дати початку та кінця змагання
  
- ⏳ **Розрахунок відсотка правильних відповідей:**
  - Для кожного закладу розраховується відсоток правильних відповідей всіх працівників
  - Формула: (загальна кількість правильних відповідей / загальна кількість питань) * 100
  - Враховується різна кількість працівників (10 в одному закладі, 16 в іншому - система справедливо порівнює відсотки)
  - Збереження результатів в FacilityCompetition (facility1CorrectPercentage, facility2CorrectPercentage)
  
- ⏳ **Визначення переможця:**
  - Переможець визначається за найвищим відсотком правильних відповідей
  - Збереження переможця в FacilityCompetition (winner: facility1/facility2/tie)
  - Оновлення результату в реальному часі під час змагання
  
- ⏳ **Статистика змагань:**
  - Кількість працівників кожного закладу (facility1EmployeesCount, facility2EmployeesCount)
  - Прогрес обох закладів з відображенням відсотків
...
- ⏳ Валідація типів питань (multiple_choice/text_input)
- ⏳ Валідація медіа файлів (фото/відео) - типи, розміри
- ⏳ Валідація правильних відповідей для text_input (масив не повинен бути порожнім)
- ⏳ Валідація доступу до бази знань перед поверненням статей
- ⏳ Валідація прив'язки статей до посад користувача
...
- ⏳ `backend/models/FacilityCompetition.js` - модель змагання між закладами
- ⏳ `backend/models/SystemSettings.js` - модель налаштувань системи
...
- ⏳ `backend/routes/facilityCompetitions.js` - роути змагань між закладами
- ⏳ `backend/routes/settings.js` - роути налаштувань системи
...
- ⏳ `backend/services/facilityCompetitionService.js` - логіка змагань між закладами
- ⏳ `backend/services/systemSettingsService.js` - логіка налаштувань системи (доступ до бази знань, ліміт часу на тест)
- ⏳ `backend/utils/levenshteinDistance.js` - алгоритм Левенштейна для перевірки текстових відповідей
- ⏳ `backend/utils/answerValidator.js` - валідація відповідей (для multiple_choice та text_input)
...
- ⏳ `backend/utils/videoUpload.js` - утиліти для роботи з відео файлами
- ⏳ `backend/utils/avatarUpload.js` - утиліти для завантаження аватара користувача
